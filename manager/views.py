...
def userlist(request):
    ...
    if request.method == "POST":
        ...
        if user_obj:
            ...
            User.objects.create(username=user, password=user[-6:])
...
def write_to_apply(request):
    if request.POST.get('choicelist','') != '':
        ...
        for user in choicelist:
            ...
            pro_info = UserProInfo.objects.get()
    ...
    if request.POST.get('choicelist','') != '':
        ...
        for user in choicelist:
            ...
            if ProMiddle.objects.filter(pro_id=pro_info.id):
                middle_info = ProMiddle.objects.get(pro_id=pro_info.id)
    ...
    if request.POST.get('choicelist','') != '':
        ...
        for user in choicelist:
            ...
            if ProConclude.objects.filter(pro_id=pro_info.id):
                conclude_info = ProConclude.objects.get(pro_id=pro_info.id)
